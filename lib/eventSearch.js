...
function relatedTitle(a, b) {
  ...
  // one title inside the other, e.g. "The X" / "Venue: X" vs "X" (shorter must be 2+ words)
  const shorter = x.length <= y.length ? x : y, longer = shorter === x ? y : x;
...
// Keep venue names out of titles - the venue has its own field
function cleanTitle(t) {
  let s = (t || "").trim();
  const isVenue = (v) => MUSIC_VENUES.test((v || "").replace(/[.']/g, ""));
  // "Venue: X", "Venue - X", "Venue presents X"
  let m = s.match(/^(.{3,40}?)\s*(?::|\s[-\u2013\u2014]\s|\spresents\s)\s*(.+)$/i);
  if (m && isVenue(m[1])) s = m[2];
  // "X @ anything"
  m = s.match(/^(.+?)\s+@\s+.+$/);
  if (m) s = m[1];
  // "X at Venue" / "X LIVE at Venue" - known venues only, 2+ words must remain
  m = s.match(/^(.+?)\s+(?:live\s+)?at\s+(.+)$/i);
  if (m && isVenue(m[2]) && m[1].trim().split(/\s+/).length >= 2) s = m[1];
  // leftover filler
  s = s.replace(/\s+(live|full band)\s*!*$/i, "").replace(/\s*!+$/, "").trim();
  return s || t;
}
...
export async function runFetch({ serpApiKey, supabaseUrl, supabaseKey, log = () => {}, concurrency = 8 }) {
  ...
  // events deleted in the admin panel - never re-add them
  const blocked = await fetch(supabaseUrl + "/rest/v1/blocked_events?select=title,location,starts_at", { headers }).then((r) => r.json()).catch(() => []);
  if (Array.isArray(blocked)) { for (const b of blocked) addSeen(b.title, b.location, b.starts_at); log("Blocked: " + blocked.length); }
  ...
  for (const e of allEvts) {
    ...
    // skip anything more than 60 days out — nobody plans that far ahead
    if (sdEarly) {
      const daysOut = (new Date(sdEarly).getTime() - Date.now()) / 86400000;
      if (daysOut > 60) { skipped++; continue; }
    }
